...
export function transformBomDataToTree(bomData: BomApiNode[]): TreeNode | null {
    ...
    const rootNode = bomData.find(node => node.level === 0 || !node.parentId || node.parentId === null);
 ...
 * 동적 트리 크기 계산
 */
export function calculateOptimalTreeSize(treeData: TreeNode, baseWidth: number, baseHeight: number) {
