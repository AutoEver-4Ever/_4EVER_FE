...
import { transformBomDataToTree, BomApiNode } from '../utils/bomTreeUtils';
    ...
    useEffect(() => {
        const processBomData = () => {
            try {
                ...
                const bomApiData: BomApiNode[] = bomData.map(node => ({
                    id: node.id,
                    code: node.code,
                    name: node.name,
                ...
                // 데이터를 트리 구조로 변환
                const transformedData = transformBomDataToTree(bomApiData);
